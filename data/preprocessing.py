"""Feature engineering and normalization for GWAS summary statistics."""
...
def process_chromosome(df: pd.DataFrame) -> pd.DataFrame:
    """Engineer features for a single chromosome dataframe."""
    df = df.copy()
    ...
    # ── Numeric features ───────────────────────────────────────────────
    # BP normalized within chromosome
...
def compute_normalization_stats() -> dict:
    """Compute mean/std from a 1% sample across all chromosomes."""
    samples = []
...
def process_and_save_all():
    """Process all chromosomes: engineer features, compute normalization, save."""
    config.DATA_PROCESSED_DIR.mkdir(parents=True, exist_ok=True)

    # First pass: feature engineering on all chromosomes
    ...
    for chrom in tqdm(range(1, 23), desc="Processing chromosomes"):
        ...
        df = process_chromosome(df)
        out_path = config.DATA_PROCESSED_DIR / f"chr{chrom}_features.parquet"
    ...
    for chrom in tqdm(range(1, 23), desc="Normalizing"):
        ...
        out_path = config.DATA_PROCESSED_DIR / f"chr{chrom}_normalized.parquet"
        df.to_parquet(out_path, index=False)
